...
from models.maskt import MASKT
...
def main(config):
    ...
    if train_config.sequence_option == "recent":  
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(users)):
        ...
        elif model_name == "maskt":
            model_config = config.maskt_config
            model = MASKT(num_skills, num_questions, seq_len, **model_config)
        ...
        train_user = train_users[:offset]
        train_user_list = list(train_user)
        test_users_list = list(test_users[:int(len(test_users)/3)])
        train_user_list.extend(test_users_list)
        train_users = np.array(train_user_list)
        ...
        if "cl" in model_name:  
            ...
            valid_loader = accelerator.prepare(
                DataLoader(
                    SimCLRDatasetWrapper(
                        valid_dataset, seq_len, 0, 0, 0, 0, eval_mode=True
                    ),
            ...
            test_loader = accelerator.prepare(
                DataLoader(
                    SimCLRDatasetWrapper(
                        test_dataset, seq_len, 0, 0, 0, 0, eval_mode=True
                    ),
                    batch_size=eval_batch_size,
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="The name of the model to train. \
            The possible models are in [akt, maskt]. \
            The default model is maskt.",
    )
    parser.add_argument(
        "--data_name",
        type=str,
    ...
    parser.add_argument(
        "--permute_prob", type=float, default=0.5, help="permute probability"
    )
    parser.add_argument(
        "--replace_prob", type=float, default=0.1, help="replace probability"
    )
    parser.add_argument(
        "--negative_prob",
        type=float,
    ...
    parser.add_argument("--l2", type=float, default=0.001, help="l2 regularization param")
    parser.add_argument("--lr", type=float, default=0.001, help="learning rate")
    parser.add_argument("--optimizer", type=str, default="adam", help="optimizer")
    parser.add_argument("--num_features", type=str, default="adam", help="optimizer")
    ...
    if args.model_name == "maskt":
        cfg.maskt_config.reg_cl = args.reg_cl
        cfg.maskt_config.mask_prob = args.mask_prob
        cfg.maskt_config.crop_prob = args.crop_prob
        cfg.maskt_config.permute_prob = args.permute_prob
        
        cfg.maskt_config.negative_prob = args.negative_prob
        cfg.maskt_config.dropout = args.dropout
        cfg.maskt_config.l2 = args.l2
    else:  
